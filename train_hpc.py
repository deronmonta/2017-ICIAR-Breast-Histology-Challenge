...
img_size = 384
num_channels = 3
train_path=r'C:\Users\FurryMonster Yang\Documents\Python Scripts\subimage'
...
num_filters_conv11 = 512

filter_size_conv12 = 3
num_filters_conv12 = 512
...
max_pool1 = tf.nn.max_pool(value=layer_conv3, 
                            ksize=[1, 2, 2, 1],
                            strides=[1, 2, 2, 1],
                            padding='SAME')

layer_drop1 = tf.layers.dropout (
    max_pool1,
    rate=0.3,
    noise_shape=None,
    training=True,
...
layer_conv4= create_convolutional_layer(input=layer_drop1,
...
max_pool2 = tf.nn.max_pool(value=layer_conv6, 
                            ksize=[1, 2, 2, 1],
                            strides=[1, 2, 2, 1],
                            padding='SAME')

layer_drop2 = tf.layers.dropout (
    max_pool2,
    rate=0.3,
    noise_shape=None,
    training=True,
...
layer_conv7= create_convolutional_layer(input=layer_drop2,
...
max_pool3 = tf.nn.max_pool(value=layer_conv9, 
                            ksize=[1, 2, 2, 1],
                            strides=[1, 2, 2, 1],
                            padding='SAME')
layer_drop3 = tf.layers.dropout (
    max_pool3,
    rate=0.3,
    noise_shape=None,
    training=True,
...
layer_conv10= create_convolutional_layer(input=layer_drop3,
...
layer_conv12= create_convolutional_layer(input=layer_conv11,
               num_input_channels=num_filters_conv11,
               conv_filter_size=filter_size_conv12,
               num_filters=num_filters_conv12)

max_pool4 = tf.nn.max_pool(value=layer_conv12, 
                            ksize=[1, 2, 2, 1],
                            strides=[1, 2, 2, 1],
                            padding='SAME')
...
layer_flat = create_flatten_layer(max_pool4)
...
summary = tf.summary.scalar('summary',y_pred)
...
def train(num_iteration):
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples/batch_size) == 0: 
            ...
            saver.save(session, 'Breast_model_aug5') 
